...
setwd("~/R/meSIT_seq_and_scEpiAge_validation/PCA")
...
library(gridExtra) #for arranging the combined plot
```

# PCA analysis of Custom library samples

## Load data from COV-files

The data used in this analysis is from the Custom library samples. The other libraries can be analysed by changing the library in the variable myInputFolder.
```{r load data}
...
#myInputFolder = "../coverage_analysis/COV-files/S185" #No enrichment
#myInputFolder = "../coverage_analysis/COV-files/S184" #xGen Nextera
myInputFolder = "../coverage_analysis/COV-files/S183" #Custom
```

```{r data handling functions, echo=FALSE, warning=FALSE, message=FALSE}
###################################
...
plot1 <- function(pc_scores_list){
  ...
  ggplot() +
    ...
    labs(title = paste("PCA of methylation-% of",n_sites, "sites"),x = "Principal Component 1", y = "Principal Component 2",
