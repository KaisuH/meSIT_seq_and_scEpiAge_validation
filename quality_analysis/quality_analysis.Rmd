...
library(knitr)
library(readr)
library(reshape2)# Melting the data for use with ggplot2
...
# Tapestation of the libraries

## Tapestation of final libraries

After enrichment (Control no enrichment):
```{r}

# Reading the data
electropherogram_data <- read.csv("Data\\Tapestation\\Tapestation_Library_samples_Electropherogram.csv", check.names = FALSE)
# Assuming each row corresponds to a sequential size or time point
electropherogram_data$Size <- 1:nrow(electropherogram_data)

electropherogram_melted <- melt(electropherogram_data[2:5], id.vars = "Size")


get_ladder_peaks <- function(electropherogram_data){
  # Known ladder sizes and their corresponding positions
  # Replace PeakPositions with your actual data
  known_sizes <- c(15, 100, 250, 400, 600, 1000, 1500, 2500, 3500, 5000, 10000)
  # Assuming your ladder data is in the first column, adjust if needed
  ladder_signal <- as.numeric(electropherogram_data[[1]])
  
  # Smooth the signal to reduce noise before finding peaks
  # Adjust the span as needed for your data
  smoothed_signal <- c(smooth(ladder_signal))
  
  # Finding peaks - adjust the threshold as necessary
  # The 'nups' and 'ndowns' define the sensitivity of peak detection
  peaks <- pracma::findpeaks(smoothed_signal, npeaks=11, sortstr = FALSE)
  
  # Filtering peaks based on a threshold - adjust as necessary
  # This threshold depends on your data's specific characteristics
  threshold <- mean(smoothed_signal) + sd(smoothed_signal)
  significant_peaks <- peaks[peaks[, 2] > threshold, ]
  
  # Extracting the positions of the significant peaks
  peak_positions <- significant_peaks[, 1]
  # A function to find peaks manually
  # Calculate the gradient (first derivative)
  gradient <- diff(smoothed_signal, differences = 1)
  
  # Find where the gradient changes sign (zero-crossings could indicate peaks)
  zero_gradient_positions <-which(diff(sign(gradient)) != 0)
  # Adjust the positions to account for the length difference caused by diff
  adjusted_positions <- zero_gradient_positions + 1
  
  # Filter these positions for signal values greater than 100
  significant_positions <- adjusted_positions[ladder_signal[adjusted_positions] > 100]
  # Adjust the range to your specific requirements
  peak_range <- 20
  # Initialize an empty vector to store the filtered positions
  filtered_positions <- numeric()
  # Iterate through the significant positions
  for (position in significant_positions) {
      # Define the range to search for neighboring peaks
      lower_bound <- max(position - peak_range, 1)
      upper_bound <- min(position + peak_range, length(ladder_signal))
  
      # Find the highest peak within this range
      range_peaks <- significant_positions[significant_positions >= lower_bound & significant_positions <= upper_bound]
      if (length(range_peaks) > 0) {
          highest_peak_position <- range_peaks[which.max(ladder_signal[range_peaks])]
          
          # Add the highest peak to the filtered list if it's not already included
          if (!(highest_peak_position %in% filtered_positions)) {
              filtered_positions <- c(filtered_positions, highest_peak_position)
          }
      }
  }
  
  significant_breaks <- list()
  significant_labels <- list()
  for (i in 1:length(filtered_positions)) {
      significant_breaks[[i]] <- filtered_positions[i]
      significant_labels[[i]] <- known_sizes[i]
  }
  return(list(significant_breaks=significant_breaks, significant_labels=significant_labels))
}

results <-get_ladder_peaks(electropherogram_data)
significant_breaks <- results$significant_breaks
significant_labels <- results$significant_labels
# Plotting the data
Library_electropherogram<-ggplot(data = electropherogram_melted, aes(x = Size, y = value, color = variable)) +
  geom_line()+
  scale_x_continuous(breaks = unlist(significant_breaks), labels = unlist(significant_labels),expand = c(0,0)) +
  scale_color_manual(values = c(
        "#8da0cb","#a13d63","#fc8d62"))+
  labs(x = "Size (bp)", y = "Signal Intensity", title = "Electropherogram after enrichment", color="Library") +
  theme_minimal() +
  ylim(0,600)+
...
Library_electropherogram
```
After tagmentation and repair:

```{r}
# Reading the data
electropherogram_data <- read.csv("Data\\Tapestation\\Library_after_tagmentation_Electropherogram.csv", check.names = FALSE)
# Assuming each row corresponds to a sequential size or time point
electropherogram_data$Size <- 1:nrow(electropherogram_data)
electropherogram_data <- electropherogram_data[order(names(electropherogram_data))]
electropherogram_melted <- melt(electropherogram_data[2:6], id.vars = "Size")
...
Library_electropherogram<-ggplot(data = electropherogram_melted, aes(x = Size, y = value, color = variable)) +
  geom_line()+
  scale_x_continuous(breaks = unlist(significant_breaks), labels = unlist(significant_labels),expand = c(0,0)) +
  scale_color_manual(values = c("#68CBAC","#4DAF4A","#FFC425","#FDBF6F"))+
  labs(x = "Size (bp)", y = "Signal Intensity", title = "Electropherogram after repair", color="Library") +
  theme_minimal() +
  ylim(0,600)+
...
```{r functions to plot per base qualities, echo=FALSE, message=FALSE}
# Function to get filenames
get_filenames <- function(directory, filepattern) {
  # List all files in the specified directory
  all_files <- list.files(path = directory, full.names = TRUE, recursive = TRUE)
  # Filter files that contain library and filetype in their names
  target_files <- grep(filepattern, all_files, value = TRUE)
  
  return(target_files)
}

# Function to read and extract quality scores
read_quality_scores <- function(file_path) {
  # Read file
  lines <- readLines(file_path)
  #print(lines)
  # Find the start and end of the "Per base sequence quality" section
  start <- which(grepl(">>Per base sequence quality", lines)) +1
  end <- which(grepl(">>END_MODULE", lines))[2] -1
  
  # Extract and read the data
  quality_data <- read.table(text = lines[start:end], header = FALSE, sep="\t", col.names = c("Base", "Mean", "Median", "LowerQuartile", "UpperQuartile", "10thPercentile", "90thPercentile"))
  return(quality_data)
}

# Function to process multiple files and combine data
process_files <- function(file_paths) {
  all_data <- list()

  for (file_path in file_paths) {
    data <- read_quality_scores(file_path)
    
    #extract the sample name
    extracted_name <- gsub(".*\\(.*)_fastqc_data.*", "\\1", file_path)

    all_data[[extracted_name]] <- data
  }
  
  all_data <- lapply(seq_along(all_data), function(i) {
    all_data[[i]]$SampleID <- names(all_data[i])
    all_data[[i]]$BaseNr <- as.numeric(gsub("([0-9]+)-.*", "\\1",all_data[[i]]$Base))
    return(all_data[[i]])
    })
...
# Plotting
base_quality <- function(combined_data, library,datatype) {
  colors <- c("#627db8ff", "#893454ff", "#fb5f22ff")
  colors <- setNames(colors, c("Custom library", "xGen Nextera library", "Control library"))
  ggplot(combined_data, aes(x = BaseNr, y = Mean, group=SampleID)) + 
    scale_x_continuous(expand = c(0,0),limits = c(1, max(combined_data$BaseNr))) +
    geom_rect(aes(xmin = 1, xmax = Inf, ymin = 30, ymax = Inf), fill = "#a6e0ceff") +
    geom_line(color=colors[library]) +
    #geom_errorbar(aes(ymin = LowerQuartile, ymax = UpperQuartile), width = 0.2) +
    theme_minimal() +
    ylim(0,40)+
    labs(title = paste("Per base sequence quality",library,datatype), x = "Base position", y = "Quality score")+
...
# R1 Val 1
```{r C_1_val_1}
#192 samples * 3 libraries * 2 reads * 2 quality scorings
# Usage with the euler exorts folde
fqc_path <- "P:\\4Students\\KaisuHiltunen\\euler_exports\\FastQC_files\\qc_extracted_data\\"
file_names <- get_filenames(fqc_path, filepattern="_C_1_val_1_fastqc")

# Process files
# quality_data_C_1_val_1<-quality_data
quality_data_C_1_val_1 <- process_files(file_names)
# Write in file
write.table(quality_data_C_1_val_1, 
            file = "Data/QC_files/FastQC_files/per_base_quality_C_1_val_1.txt", 
            sep = "\t", 
            row.names = FALSE, 
            col.names = TRUE)

# Filter lambda
quality_data_filtered_C <- quality_data_C_1_val_1 %>%
  filter(!grepl("La", SampleID))
# Plot data
base_quality(quality_data_filtered_C, 
             library = "Custom library", 
             datatype = "\nR1 trimmed")
```
```{r X_1_val_1}
file_names <- get_filenames(fqc_path, filepattern="_X_1_val_1_fastqc")

# Process files
#quality_data_C_1_val_1<-quality_data
quality_data_X_1_val_1 <- process_files(file_names)
# Write in file
write.table(quality_data_X_1_val_1, 
            file = "Data/QC_files/FastQC_files/per_base_quality_X_1_val_1.txt", 
            sep = "\t", 
            row.names = FALSE,
            col.names = TRUE)
# Plot data
base_quality(quality_data_X_1_val_1, "xGen Nextera library", "\nR1 trimmed")
```
```{r N_1_val_1}
file_names <- get_filenames(fqc_path, filepattern = "_N_1_val_1_fastqc")

# Process files
quality_data_N_1_val_1 <- process_files(file_names)
# Write in file
write.table(quality_data_N_1_val_1, 
            file = "Data/QC_files/FastQC_files/per_base_quality_N_1_val_1.txt", 
            sep = "\t", 
            row.names = FALSE,
            col.names = TRUE)

quality_data_filtered_N <- quality_data_N_1_val_1 %>%
  filter(!grepl("La", SampleID))
# Plot data
base_quality(quality_data_filtered_N, "Control library", "\nR1 trimmed")
```
...
```{r control R2 val 2}
#192 samples * 3 libraries * 2 reads * 2 quality scorings
file_names <- get_filenames(fqc_path, filepattern="_C_2_val_2_fastqc")

# Process files
quality_data_C_2_val_2 <- process_files(file_names)
# Write in file
write.table(quality_data_C_2_val_2, 
            file = "Data/QC_files/FastQC_files/per_base_quality_C_2_val_2.txt", 
            sep = "\t", 
            row.names = FALSE, 
            col.names = TRUE)
# Plot data
base_quality(quality_data_C_2_val_2, "Custom library", "\nR2 trimmed")
```
```{r library X 2_val_2 reads}
file_names <- get_filenames(fqc_path, filepattern="_X_2_val_2_fastqc")

# Process files
quality_data_X_2_val_2 <- process_files(file_names)
# Write in file
write.table(quality_data_X_2_val_2, 
            file = "Data/QC_files/FastQC_files/per_base_quality_X_2_val_2.txt", 
            sep = "\t", 
            row.names = FALSE,
            col.names = TRUE)
# Plot data
base_quality(quality_data_X_2_val_2, "xGen Nextera library", "\nR2 trimmed")
```
```{r library N 1_val_1 reads}
file_names <- get_filenames(fqc_path, filepattern="_N_2_val_2_fastqc")

# Process files
quality_data_N_2_val_2 <- process_files(file_names)
# Write in file
write.table(quality_data_N_2_val_2, 
            file = "Data/QC_files/FastQC_files/per_base_quality_N_2_val_2.txt", 
            sep = "\t", 
            row.names = FALSE,
            col.names = TRUE)
# Plot data
base_quality(quality_data_N_2_val_2, "Control library", "\nR2 trimmed")
```
...
```{r}
#192 samples * 3 libraries * 2 reads * 2 quality scorings
file_names <- get_filenames(fqc_path, filepattern="_C_1_fastqc")

# Process files
quality_data_C_1 <- process_files(file_names)
# Write in file
write.table(quality_data_C_1, 
            file = "Data/QC_files/FastQC_files/per_base_quality_C_1.txt", 
            sep = "\t", 
            row.names = FALSE, 
            col.names = TRUE)

quality_data_filtered_C_1 <- quality_data_C_1 %>%
  filter(!grepl("La", SampleID))
# Plot data
base_quality(quality_data_filtered_C_1, "Custom library", "\nR1")
```
```{r library X 1_val_1 reads}
file_names <- get_filenames(fqc_path, filepattern="_X_1_fastqc")

# Process files
quality_data_X_1 <- process_files(file_names)
# Write in file
write.table(quality_data_X_1, 
            file = "Data/QC_files/FastQC_files/per_base_quality_X_1.txt", 
            sep = "\t", 
            row.names = FALSE,
            col.names = TRUE)
# Plot data
base_quality(quality_data_X_1, "xGen Nextera library", "\nR1")
```
```{r library N 1_val_1 reads}
file_names <- get_filenames(fqc_path, filepattern="_N_1_fastqc")

# Process files
quality_data_N_1 <- process_files(file_names)
# Write in file
write.table(quality_data_N_1, 
            file = "Data/QC_files/FastQC_files/per_base_quality_N_1.txt", 
            sep = "\t", 
            row.names = FALSE,
            col.names = TRUE)
# Plot data
base_quality(quality_data_N_1, "Control library", "\nR1")
```
...
```{r custom 2 reads}
#192 samples * 3 libraries * 2 reads * 2 quality scorings
file_names <- get_filenames(fqc_path, filepattern="_C_2_fastqc")

# Process files
quality_data_C_2 <- process_files(file_names)
# Write in file
write.table(quality_data_C_2, 
            file = "Data/QC_files/FastQC_files/per_base_quality_C_2.txt", 
            sep = "\t", 
            row.names = FALSE, 
            col.names = TRUE)

quality_data_filtered_C_2 <- quality_data_C_2 %>%
  filter(!grepl("La", SampleID))
# Plot data
base_quality(quality_data_filtered_C_2, "Custom library", "\nR2")
```
```{r xGen 2 reads}
file_names <- get_filenames(fqc_path, filepattern="_X_2_fastqc")

# Process files
quality_data_X_2 <- process_files(file_names)
# Write in file
write.table(quality_data_X_2, 
            file = "Data/QC_files/FastQC_files/per_base_quality_X_2.txt", 
            sep = "\t", 
            row.names = FALSE,
            col.names = TRUE)
# Plot data
base_quality(quality_data_X_2, "xGen Nextera library", "\nR2")
```
```{r Control 2 reads}
file_names <- get_filenames(fqc_path, filepattern="_N_2_fastqc")

# Process files
quality_data_N_2 <- process_files(file_names)
# Write in file
write.table(quality_data_N_2, 
            file = "Data/QC_files/FastQC_files/per_base_quality_N_2.txt", 
            sep = "\t", 
            row.names = FALSE,
            col.names = TRUE)
# Plot data
base_quality(quality_data_N_2, "Control library", "\nR2")
```
