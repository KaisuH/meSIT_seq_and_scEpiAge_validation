...
# Create the combined df
combined_df <- rbind(C_df,X_df,N_df)
...
selected_columns <- c("Aligned Uniquely", "Aligned Ambiguously", "Total reads before deduplication", "Deduplicated reads (remaining)", "Library")
...
plot_summary_combined <- function(data) {
  ...
  ggplot(data, aes(x = `Variable`, 
    ...
    scale_fill_manual(values = c("#2978a0", "#710C32", "#fc8d62")) +
    scale_color_manual(values = c("#216080", "#590a27", "#b36446")) +
    labs(title = paste0("Processing of all libraries"),
         y = "Percentage of reads") +
    theme_minimal() +
    guides(fill = guide_legend(nrow = 3)) +
    theme(legend.background = element_rect(fill = "white", 
                                           colour = "grey", 
                                           linewidth = 0.5),
          legend.justification = c(0, 0), # legend position to the top-right
          legend.position = c(0.0, 0.0),      # Position legend inside the plot
          axis.title.x = element_blank(), # Remove x-axis title
          panel.grid.minor.x = element_blank(), # Remove minor vgridlines
