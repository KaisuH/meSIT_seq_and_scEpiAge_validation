...
replicate_boxplot <- function(data){
  ...
  ggplot(data, aes(x = sampleName, y = predictedAge)) +
    geom_boxplot() +
    geom_text(data = unique_samples, aes(label = "*", x = sampleName, y = actualAge)
              , color="#2978a0", size=6) +
    theme_minimal() +
...
replicate_boxplot(blood_clock_data)
```

Only blood sample coparison. Blood samples had all three replicates.
