...
## Read enrichment non logarithmisc scale

```{r}
#"#2978a0", "#68CBAC", "#710C32", "#fc8d62", "#F768A1", "#FDBF6F", 
#'#88CCEE', "black", "#E41A1C", "#FF7F00","#4DAF4A", "#FFC425",
# "#5E3C99", "#1F78B4", "grey",'#CC6677', '#332288'

library(ggplot2)
library(dplyr)

# Add a library column to each dataframe
C_data$library <- "Custom"
X_data$library <- "xGen\nNextera"
N_data$library <- "Control"

# Combine into one dataframe
combined_df <- rbind(C_data, X_data, N_data)

# Ensure 'library' is a factor with levels in the order you want
combined_df$library <- factor(
  combined_df$library, 
  levels = c("Custom", "xGen\nNextera", "Control")
) 

# Ensure 'category' is a factor if not already
combined_df$category <- as.factor(combined_df$category)

# Custom colors
custom_colors <- c("#fc8d62", "#68CBAC", "#E41A1C","#4d4d4dff")

# Plot the stacked bar plot
read_type_plot <- ggplot(combined_df, aes(x = library, y = value, fill = category)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = custom_colors) +
  labs(title = "Read type distribution per library",
       #x = "Library",
       y = "Reads",
       fill = "Category"
       ) +
...
colnames(library_read_depth) <- gsub("_", " ", 
                                     gsub("Mean ", "", 
                                          colnames(library_read_depth)))

# Extract only necessary columns
library_read_depth<-library_read_depth[,c(7,9,11)]

# Rename columns for uniformity
colnames(library_read_depth) <- c("Custom", "xGen Nextera", "Control")

plot_data<-library_read_depth%>%
  stack()%>%
  mutate(ind = fct_reorder(ind, values, .fun = 'median'))

colnames(plot_data) <- c("Values","Library")
...
# Convert library to factor with custom order
plot_data$Library <- factor(plot_data$Library, 
                            levels = c("Custom", "xGen Nextera", "Control"))

# Create the violin plot
violin_plot<-ggplot(plot_data, 
                    aes(x = Library, y = Values, fill = Library)) +
  geom_violin(trim = TRUE,
              ...
              adjust=0.5) +
  scale_fill_manual(values = c("Custom" = "#8da0cb", 
                               "xGen Nextera" = "#a13d63", 
                               "Control" = "#fc8d62")) +
  labs(x = "Library", 
       y = "Read depth\n(Per million reads)", 
       title = "Clock site read depth") +
  theme_minimal() +
...
```{r wilcoxon sum test}
wilcox_results <- wilcox.test(depth_C, depth_X, paired = TRUE, alternative="greater", exact=FALSE, conf.int = TRUE)
wilcox_results
...
# Calculate Cohen's d
d_value <- cohens_d(depth_C, depth_X)
print(d_value)
```
